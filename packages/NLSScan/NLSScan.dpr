...
begin
  ...
  begin
    ...
    if bScanWinDir then
    begin
      strQuestion := Format('Have %d bad NLS files. Do you want to copy it to TEMP directory %s and delete them?', [totalBad, g_strTempPath]);
      ...
      begin
        ...
        try
          ...
          if CopyFile(PChar(strPath), PChar(strNewPath), False) then
            WriteToLogAndConsole(@logFile, Format('File %s copied to %s'#13#10, [strPath, strNewPath]), bScanWinDir)
          else
            WriteToLogAndConsole(@logFile, Format('Could not copy file %s to %s'#13#10'Error: %s'#13#10, [strPath, strNewPath, SysErrorMessage(GetLastError)]), bScanWinDir);
...
begin
  ...
  g_strTempPath := TPath.Combine(TPath.GetTempPath, 'NLSScan');

  try
    TDirectory.CreateDirectory(g_strTempPath);
  except
    WriteLn('Create Directory Error.');
  end;
  ...
  try
    Intro;
    Scan;
    CopyFile(PChar(TPath.Combine(g_strExeDir, TPath.ChangeExtension(g_strExeName, '.log'))), PChar(TPath.Combine(g_strTempPath, TPath.ChangeExtension(g_strExeName, '.log'))), False);
  except
    on E: Exception do
